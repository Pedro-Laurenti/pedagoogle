...
import {
  MdArrowBack, MdAdd, MdDelete, MdPictureAsPdf, MdDescription,
  MdAssignmentTurnedIn, MdArrowUpward, MdArrowDownward, MdTextFields,
} from "react-icons/md";
import { invokeCmd } from "@/utils/tauri";
import { save } from "@tauri-apps/plugin-dialog";
import RichEditor from "@/components/RichEditor";
import "katex/dist/katex.min.css";
import type { Materia, Prova, Questao, QuestaoInput, OpcaoQuestao, TipoQuestao, Turma, ToastState } from "@/types";
...
const EMPTY_FORM: ProvaForm = {
  descricao: "", materia_id: "", bimestre: "1",
  ano_letivo: new Date().getFullYear().toString(),
  valor_total: "10", turma_id: "",
};
...
function gerarTituloPreview(materias: Materia[], materiaId: string, bimestre: string) {
  const mat = materias.find((m) => String(m.id) === materiaId);
  if (!mat) return "";
  return `AVALIAÇÃO DE ${mat.nome.toUpperCase()} - ${bimestre}º BIMESTRE`;
}

export default function ProvaEditor({ provaId, materias, turmas, usarTurmas, onClose, onNotify }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setForm({
      ...
      bimestre: String(prova.bimestre ?? 1),
      ano_letivo: prova.ano_letivo ?? "",
  ...
  function set(field: keyof ProvaForm, value: string) {
  ...
  function validate(): boolean {
    ...
    if (!form.valor_total || isNaN(parseFloat(form.valor_total))) errs.valor_total = "Informe o valor total";
    setErrors(errs);
  ...
  async function handleSave() {
    ...
    try {
      const payload = {
        ...
        valorTotal: parseFloat(form.valor_total) || 10,
      };
      let id = provaId;
      if (!id) {
  ...
  const tituloPreview = gerarTituloPreview(materias, form.materia_id, form.bimestre);
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold">{provaId ? "Editar Prova" : "Nova Prova"}</h1>
          {tituloPreview && (
            <p className="text-sm text-base-content/60 mt-0.5">{tituloPreview}</p>
          )}
        </div>
        {provaId && (
          <>
      ...
      <div className="tabs tabs-boxed mb-6 w-fit">
        <button
          className={`tab ${aba === "config" ? "tab-active" : ""}`}
          onClick={() => setAba("config")}
        >
          Configurações
        </button>
        <button
          className={`tab ${aba === "questoes" ? "tab-active" : ""}`}
          onClick={() => setAba("questoes")}
        >
          Questões
          {numQuestoes > 0 && (
            <span className="badge badge-sm ml-2">{numQuestoes}</span>
          )}
        </button>
      </div>
      ...
      {aba === "questoes" && (
        <>
          ...
          {!somaOk && questoes.filter((q) => q.tipo !== "texto").length > 0 && (
